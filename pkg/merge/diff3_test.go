...
func TestText(t *testing.T) {
	Convey("Given a table of three-way text merges", t, func() {
		...
		for name, tt := range tests {
			Convey("When "+name, func() {
				...
				Convey("Then the merge matches", func() {
					So(got.Conflicts, ShouldEqual, tt.wantConflicts)

					if tt.wantConflicts == 0 || tt.want != "" {
						So(string(got.Merged), ShouldEqual, tt.want)
					}
				})
			})
		}
