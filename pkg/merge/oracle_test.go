...
func TestTextOracle(t *testing.T) {
	...
	Convey("Given the git merge-file oracle", t, func() {
		...
		for name, tt := range tests {
			Convey("When "+name, func() {
				...
				So(os.WriteFile(basePath, []byte(tt.base), 0o600), ShouldBeNil)
				So(os.WriteFile(vaultPath, []byte(tt.vault), 0o600), ShouldBeNil)
				So(os.WriteFile(agentPath, []byte(tt.agent), 0o600), ShouldBeNil)
				...
				Convey("Then the merge matches git", func() {
					So(string(got.Merged), ShouldEqual, string(expected))
				})
			})
		}
	})
